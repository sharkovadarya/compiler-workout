...
*)                         
let rec eval env (w, st, (s, i, o)) p = match p with
  | [] -> (w, st, (s, i, o))
  | inst :: p' -> match inst with
    ...
    | BEGIN (args, locals) -> let c = (w, st, (s, i, o)) in 
                              let (w, st, (c, i, o)) = c in
                              let c = State.push_scope c (args @ locals) in
                                eval env (put_args args (w, st, (c, i, o))) p'
    | END -> (match w with
      | [] -> (w, st, (s, i, o))
      | (p, s') :: w' -> eval env (w', st, (State.drop_scope s s', i, o)) p)
...
let rec compile' i =
  ...
  | Stmt.Call (f, l) -> List.concat (List.map expr l) @ [CALL f], i
