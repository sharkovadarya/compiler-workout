...
let rec compile' i =
  ...
  | Expr.Call (f, l)      -> List.fold_left (fun p l -> expr l @ p) [CALL (f, List.length l, true)] l  
  in
  function
  ...
  | Stmt.Call (f, l) -> expr (Expr.Call (f, l)), i
  | Stmt.Return None     -> [RET false], i
  | Stmt.Return (Some e) -> (expr e) @ [RET true], i
...
let rec compile_functions i = function
    ...
    | (name, (args, locals, body)) :: l -> let compiled_body, i = compile' i body in
                                           let r, i = compile_functions i l in
                                             [LABEL name] @ [BEGIN (name, args, locals)] @ compiled_body @ [END] @ r, i
